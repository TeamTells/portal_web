...
export class AuthorizationReducer
  implements Reducer<AuthorizationState, AuthorizationResultAction>
{
  reduce(
    state: AuthorizationState,
    action: AuthorizationResultAction
  ): AuthorizationState {
    switch (action.type) {
      case AuthorizationResultActionTypes.TOGGLE_REMEMBER_ME:
        return clone(state, { rememberMe: !state.rememberMe });
